...
class Schedule(Base):
    ...
    lunch_start: Mapped[time | None] = mapped_column(
        Time, nullable=True
    )  # Inicio de almuerzo
    lunch_end: Mapped[time | None] = mapped_column(
        Time, nullable=True
    )  # Fin de almuerzo
    dinner_start: Mapped[time | None] = mapped_column(
        Time, nullable=True
    )  # Inicio de cena
    dinner_end: Mapped[time | None] = mapped_column(Time, nullable=True)  # Fin de cena
    net_hours: Mapped[Decimal | None] = mapped_column(
        Numeric(5, 2), nullable=True
    )  # Horas laborales netas por jornada
...
class ScheduleAssignment(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "employee_id", "assignment_date", name="uq_employee_assignment_date"
        ),
    )
