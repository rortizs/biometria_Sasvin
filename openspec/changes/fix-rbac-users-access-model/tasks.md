...
- [ ] 3.10 RED/GREEN: `backend/app/api/v1/endpoints/schedules.py`, `departments.py`, `positions.py`, `locations.py` — replace `get_current_coordinador_or_above`/`get_current_secretaria_or_above` with `require_permission` + `assert_request_scope`; `COORDINADOR`/`DIRECTOR` read-only. Spec: "Module Access Boundaries" (DIRECTOR/COORDINADOR scenarios). **PARTIAL — `schedules.py` portion done** (this call): all 4 read endpoints (`list_schedule_patterns`, `get_schedule_pattern`, `list_assignments`, `list_exceptions`) gained `get_current_user` auth (2 had none at all — closed a real pre-existing gap); all 9 write endpoints (patterns create/update/delete, assignments create/bulk-create/delete, exceptions create/update/delete) swapped `get_current_secretaria_or_above`/`get_current_active_admin` for `require_permission("schedules.create"/"update"/"delete")`. No `assert_request_scope` call added — spec's "Module Access Boundaries" scenarios don't require read-scoping for schedules specifically (only for dashboard/attendance reports and employees), so none was added to avoid guessing at unspecified filtering. `departments.py`/`positions.py`/`locations.py` NOT started — remaining scope for the next apply call.
