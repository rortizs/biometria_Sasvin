    ...
    <div class="page">
      ...
      <div class="table-container">
        @if (loading()) {
          ...
          <div class="empty">No hay puestos registrados.</div>
        } @else {
          <table>
            <thead>
              <tr>
            ...
            <tbody>
              @for (pos of positions(); track pos.id) {
                <tr>
                  ...
                  <td>
                    <span class="badge" [class.success]="pos.is_active" [class.danger]="!pos.is_active">
                      {{ pos.is_active ? 'Activo' : 'Inactivo' }}
                    </span>
                  </td>
                  <td>
                    <div class="actions">
                      <button class="btn btn-sm btn-edit" (click)="openEditModal(pos)">✏️ Editar</button>
                      <button class="btn btn-sm btn-danger" (click)="deletePosition(pos)">🗑️ Eliminar</button>
                    </div>
                  </td>
                </tr>
              }
      ...
      @if (showModal()) {
        <div class="modal-overlay" (click)="closeModal()">
          <div class="modal" (click)="$event.stopPropagation()">
            <h2>{{ editMode() ? 'Editar Puesto' : 'Nuevo Puesto' }}</h2>
            <form (ngSubmit)="savePosition()">
              <div class="form-group">
                <label>Nombre *</label>
                <input [(ngModel)]="formData.name" name="name" required placeholder="Ej: Catedrático Titular" />
              </div>
              <div class="form-group">
                <label>Descripción</label>
                <textarea [(ngModel)]="formData.description" name="description" rows="3"
                  placeholder="Descripción opcional del puesto"></textarea>
              </div>
              @if (editMode()) {
                <div class="form-group">
...
export class PositionsComponent implements OnInit {
  ...
  loadPositions(): void {
    ...
    this.positionService.getPositions(false).subscribe({
  ...
  openEditModal(pos: Position): void {
    ...
    this.formData = { name: pos.name, description: pos.description || '', is_active: pos.is_active };
    this.errorMsg.set(null);
    this.showModal.set(true);
  }
  ...
  savePosition(): void {
    ...
    if (this.editMode() && this.editingId) {
      this.positionService.updatePosition(this.editingId, this.formData).subscribe({
        next: () => { this.saving.set(false); this.closeModal(); this.loadPositions(); },
        error: handleError,
      });
    } else {
      this.positionService.createPosition({ name: this.formData.name, description: this.formData.description }).subscribe({
        next: () => { this.saving.set(false); this.closeModal(); this.loadPositions(); },
        error: handleError,
      });
    }
  ...
  deletePosition(pos: Position): void {
    if (!confirm(`¿Eliminar el puesto "${pos.name}"?\nEsto puede afectar empleados asignados a este puesto.`)) return;
